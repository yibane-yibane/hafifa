...
class DataModelsTransactionsHandler:
    def __init__(self):
        self.db = SQLAlchemyHandler()
    ...
    def create_and_insert_to_database_video_metadata_frame_models(self,
        ...
        is_video_exists = self.db.is_data_model_exists(data_model=data_models.Video,
        ...
        if not is_video_exists:
            video_model = data_models.Video(observation_name=observation,
                                            ...
                                            number_of_frames=len(image_list))
            self.db.insert_one(video_model)
